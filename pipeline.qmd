...
## 6.1 Proxy Variable Methodology

Six social determinant variables included in the CDC PLACES dataset were **not available at the Oregon county level** and were therefore estimated using linear proxy formulas derived from highly correlated Census variables. These are educated approximations, not direct measurements, and are flagged as a known limitation of this analysis.

| Variable | Formula | Census Driver | Rationale |
|---|---|---|---|
| **Food Insecurity** | `% Poverty × 1.1 + 2.0` | Poverty rate | Food insecurity tracks closely with poverty but tends to run slightly higher — the multiplier (1.1) reflects this, and the +2.0 intercept captures baseline food insecurity even in low-poverty counties. |
| **Housing Insecurity** | `% Poverty × 0.9 + 1.5` | Poverty rate | Closely related to poverty, but at a slightly lower rate than food insecurity — households typically prioritize housing payments before food. |
| **Utility Shutoff Threat** | `% Poverty × 0.3 + 0.5` | Poverty rate | A more extreme outcome affecting a smaller fraction of even economically stressed households, hence the lower multiplier. |
| **Transportation Barriers** | `% No Vehicle × 1.8 + 3.0` | Vehicle access rate | Driven by vehicle access rather than poverty. The 1.8 multiplier reflects that in rural Oregon — where transit infrastructure is minimal — lacking a vehicle creates amplified access barriers. |
| **Loneliness** | `22.0 + % Over 65 × 0.2` | Elderly population share | A baseline loneliness rate of ~22% exists across all age groups; the elderly share adds a modest increment reflecting well-documented age-related social isolation. |
| **Lack of Social Support** | `14.0 + % Over 65 × 0.15` | Elderly population share | Similar structure to loneliness, with a lower baseline and slope reflecting the narrower prevalence of this specific indicator. |

> **Note:** These formulas produce estimates that are correlated with the true values but introduce imprecision into the Need Index for the five thematic categories that include them (Socioeconomic Vulnerability and Lifestyle/Behavioral Risk). The broad county-level patterns of the Need Index are robust even when these variables are excluded entirely.
